...
import { User, Subject, StudentTab, SystemSettings, CreditPackage, WeeklyTest, Chapter, MCQItem, Challenge20, ViewState } from '../types';
...
interface Props {
  ...
  onNavigate?: (view: ViewState) => void; // Updated type
              ...
              <div className="space-y-6 pb-24">
                ...
                {/* HERO SECTION */}
                  <div className="space-y-4 mb-6">
                      ...
                      {/* CUSTOM PAGE BANNER */}
                      {settings?.customPageHtml && (
                          <div 
                              onClick={() => onNavigate && onNavigate('CUSTOM_PAGE')}
                              className="mx-1 mb-4 bg-gradient-to-r from-pink-500 to-rose-500 p-4 rounded-2xl shadow-lg cursor-pointer flex items-center justify-between text-white relative overflow-hidden group"
                          >
                              <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-20"></div>
                              <div className="relative z-10">
                                  <h3 className="text-xl font-black italic mb-1">New Update</h3>
                                  <p className="text-xs font-bold text-pink-100">Check out the latest announcement!</p>
                              </div>
                              <div className="relative z-10 bg-white/20 p-2 rounded-full backdrop-blur-md group-hover:scale-110 transition-transform">
                                  <ArrowRight size={24} />
                              </div>
                          </div>
                      )}
                  ...
                  <div className="bg-gradient-to-r from-pink-50 to-rose-50 p-4 rounded-2xl border border-pink-100 shadow-sm mt-4">
                      ...
                      <div>
                          ...
                          <div className="grid grid-cols-3 gap-3">
                              ...
                              <DashboardTileWrapper id="tile_deep_analysis" label="Deep Analysis">
                              <button onClick={() => onTabChange('DEEP_ANALYSIS')} className="h-16 w-full bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
                                  <BrainCircuit size={18} style={{ color: 'var(--primary)' }} />
                                  <span className="text-[10px] font-black text-slate-700 uppercase tracking-wider">Smart Hub</span>
                              </button>
                              </DashboardTileWrapper>
