...
export const MarksheetCard: React.FC<Props> = ({ result, user, settings, onClose, onViewAnalysis, onPublish, questions, onUpdateUser, initialView }) => {
  ...
  const handleRoyalShare = async () => {
      const elementId = ['MARKSHEET_1', 'MARKSHEET_2'].includes(activeTab) ? (activeTab === 'MARKSHEET_2' ? 'marksheet-style-2' : 'marksheet-style-1') : 'marksheet-content';
      const element = document.getElementById(elementId);
      if (!element) return;
      ...
      try {
          ...
          const appLink = settings?.officialAppUrl || "https://play.google.com/store/apps/details?id=com.nsta.app";
          const text = `👑 *ROYAL MARKSHEET* 👑\n\nStudent: ${user.name}\nScore: ${result.score}/${result.totalQuestions}\nPercentage: ${percentage}%\n\nCheck my result on App! 👇\n${appLink}`;

          if (blob && navigator.share) {
              const file = new File([blob], "royal_result.png", { type: "image/png" });
          ...
          const link = document.createElement('a');
          link.download = `Royal_Result_${user.name}.png`;
          link.href = canvas.toDataURL('image/png');
          link.click();
          ...
          console.error("Royal Share Failed", e);
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-0 sm:p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in overflow-hidden">
        <div className="w-full max-w-2xl h-full sm:h-auto sm:max-h-[90vh] bg-white sm:rounded-3xl shadow-2xl flex flex-col relative overflow-hidden">
            ...
            <div className="bg-white p-4 border-t border-slate-100 flex gap-2 justify-center z-10 shrink-0 flex-col sm:flex-row">
                ...
                <button onClick={handleRoyalShare} className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-4 py-3 rounded-xl font-bold text-xs shadow hover:scale-105 transition-transform flex justify-center gap-2">
                    <Share2 size={16} /> Royal Share
                </button>
