...
import { BrainCircuit, Play, Pause, ChevronDown, ChevronUp, Star, Lock, AlertCircle, CheckCircle, XCircle, Layers, Zap } from 'lucide-react';
...
export const AiDeepAnalysis: React.FC<Props> = ({ user, settings, onUpdateUser, onBack }) => {
    ...
    const [allMistakes, setAllMistakes] = useState<{ question: string, correct: string, explanation?: string }[]>([]);
    ...
    const analyzeHistory = () => {
        ...
        const globalMistakes: { question: string, correct: string, explanation?: string }[] = [];
        ...
        history.forEach(test => {
            ...
            if (test.wrongQuestions) {
                test.wrongQuestions.forEach(wq => {
                    // Try to find correct answer from original history if available
                    // For now, we use placeholder or what we have
                    const mistakeObj = {
                        question: wq.question,
                        correct: 'Review Topic Notes',
                        explanation: 'Concept gap detected.'
                    };
                    topicMap[topicName].mistakes.push(mistakeObj);
                    globalMistakes.push(mistakeObj);
                });
            }
        });

        setAllMistakes(globalMistakes);
    ...
    const handlePlayAdvice = async (text: string) => {
        if (isPlaying) {
            stopSpeech();
            setIsPlaying(false);
        } else {
            const voices = await getCategorizedVoices();
            // Prioritize Hindi Voice for "Audio Insights" as requested
            const hindiVoice = voices.hindi[0] || voices.indianEnglish[0];
            speakText(text, hindiVoice, 1.0);
            setIsPlaying(true);
        }
    };
    ...
    return (
        <div className="pb-24 animate-in slide-in-from-bottom-4">
            {showFlashcards && (
                <FlashcardView mistakes={allMistakes} onClose={() => setShowFlashcards(false)} />
            )}
            {showMindMap && (
                <MindMapView topic={showMindMap} onClose={() => setShowMindMap(null)} />
            )}
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                            <BrainCircuit className="text-violet-600" /> Smart Hub
                        </h2>
                ...
                {allMistakes.length > 0 && (
                    <button 
                        ...
                        className="bg-gradient-to-r from-pink-500 to-rose-500 text-white px-4 py-2 rounded-xl font-bold shadow-lg hover:scale-105 transition-transform flex items-center gap-2 text-xs"
                    >
                        <Layers size={16} /> Revision Deck
                    </button>
                )}
            </div>
                            ...
                            {expandedTopic === t.topic && (
                                <div className="p-4 pt-0 border-t border-slate-100/50 bg-white/50">
                                    ...
                                    <div className="mt-4 flex gap-2 justify-center">
                                        <button className="text-xs font-bold text-blue-600 hover:underline bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
                                            View Detailed Notes
                                        </button>
                                        <button 
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setShowMindMap(t.topic);
                                            }}
                                            className="text-xs font-bold text-purple-600 hover:underline bg-purple-50 px-3 py-1.5 rounded-lg border border-purple-100 flex items-center gap-1"
                                        >
                                            <Zap size={12} /> View Mind Map
                                        </button>
                                    </div>
                                </div>
                            )}
