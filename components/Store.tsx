...
import { MegaDiscountPage } from './MegaDiscountPage';
...
export const Store: React.FC<Props> = ({ user, settings }) => {
  ...
  const [showMegaDiscount, setShowMegaDiscount] = useState(false);
      ...
      <div className="max-w-md mx-auto pt-8 px-4">
          ...
          <div className="text-center mb-8">
              ...
              {showEventBanner && timeLeft && (
                  <div 
                      onClick={() => setShowMegaDiscount(true)}
                      className={`mt-4 bg-gradient-to-r border p-4 rounded-xl relative overflow-hidden cursor-pointer ${activeEvent ? 'from-amber-900/40 to-orange-900/40 border-amber-500/30' : 'from-slate-900/40 to-slate-800/40 border-slate-500/30'}`}
                  >
              ...
              {subscriptionPlans.map(plan => {
                  ...
                  if (activeEvent) {
                      ...
                      if ((isSubscribed && event?.showToPremiumUsers) || (!isSubscribed && event?.showToFreeUsers)) {
                          // ADDITIVE DISCOUNT LOGIC: Final = Current - (Original * Mega%)
                          const megaDiscountAmount = original * (eventDiscount / 100);
                          price = Math.max(0, Math.round(price - megaDiscountAmount));
                      }
                      ...
                      const megaDiscountAmount = original * (genDiscount / 100);
                      price = Math.max(0, Math.round(price - megaDiscountAmount));
                  }
